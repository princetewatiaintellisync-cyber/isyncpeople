...
class ConveyanceClaimApiService {
  static const String _endpoint =
      'https://delton.intellisync.in:11004/claims/conveyance-claim-history/?json=1';

  final AuthService _authService = AuthService();

  Future<List<Map<String, dynamic>>> fetchConveyanceClaims() async {
    try {
      debugPrint('🔄 Fetching conveyance claims from API...');
      ...
      debugPrint('📊 Conveyance claims response status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['claims'] != null) {
          ...
          debugPrint('📋 Parsed ${claims.length} conveyance claims');
          return claims;
        } else {
          return [];
        }
      } else {
        throw Exception('Failed to fetch conveyance claims: ${response.statusCode}');
      }
    } catch (e) {
      debugPrint('❌ Error fetching conveyance claims: $e');
      rethrow;
    }
  }
