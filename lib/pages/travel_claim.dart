...
import '../utils/travel_claim_api.dart';
import '../utils/permission_service.dart';

class TravelClaimPage extends StatefulWidget {
  const TravelClaimPage({super.key});

  @override
  State<TravelClaimPage> createState() => _TravelClaimPageState();
}

class _TravelClaimPageState extends State<TravelClaimPage> {
  final TravelClaimApiService _apiService = TravelClaimApiService();
  ...
  static const Map<int, TableColumnWidth> _colWidths = {
    ...
    3: FixedColumnWidth(220), // EMPLOYEE NAME(PAYCODE)
  ...
  Future<void> _loadPermissions() async {
    final canWrite = await _permissionService.canWrite('Travel Claim');
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final data = await _apiService.fetchTravelClaims();
      if (mounted) {
        setState(() {
          claimData = data;
          filteredData = data;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to load travel claims: ${e.toString()}'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Travel Claim History'),
                        ...
                        children: [
                          Text(
                            searchQuery.isEmpty
                                ...
                                : '${filteredData.length} of ${claimData.length}',
                            style: TextStyle(
  ...
  Widget _buildDataCell(String text, {bool center = false}) {
    return Container(
